...
class ApiClient:
    ...
    def verify_connection(self):
        """Verify connection to the server and get API info"""
        try:
            response = requests.get(self.base_url, timeout=5)
            if response.ok:
                return {"status": "connected", "url": self.base_url}
            return {"status": "reachable", "url": self.base_url, "content_type": response.headers.get("Content-Type", "")}
        except Exception as e:
            return {"status": "failed", "url": self.base_url, "error": str(e)}
...
class LoginWindow(QWidget):
    ...
    def setup_ui(self):
        ...
        server_buttons_layout.addWidget(self.test_connection_button)
        layout.addLayout(server_buttons_layout)
    ...
    def on_connection_test_result(self, result):
        """Handle connection test result"""
        self.test_connection_button.setEnabled(True)
        self.test_connection_button.setText("Test Connection")
        if result['status'] == 'connected':
            QMessageBox.information(self, "Connection Successful", 
                                    f"Successfully connected to the server at {result['url']}")
        elif result['status'] == 'reachable':
            QMessageBox.warning(self, "Server Reachable", 
    ...
    def on_connection_test_error(self, error_message):
        ...
        self.test_connection_button.setEnabled(True)
        self.test_connection_button.setText("Test Connection")
        QMessageBox.critical(self, "Connection Test Failed", f"Error: {error_message}")
    ...
    def attempt_login(self):
        ...
        # Connect success and error handlers
        self.worker.finished.connect(self.on_login_successful)
...
class MainWindow(QMainWindow):
    ...
    def on_login_successful(self, response):
        ...
        username = response.get('user', {}).get('username', 'User')
    ...
    def on_auto_login_success(self):
        ...
        self.connection_status.setStyleSheet("color: green;")
        # Show welcome message
        username = self.api_client.get_saved_credentials()[0]
        QMessageBox.information(self, "Welcome", f"Welcome, {username}!")
